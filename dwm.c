...
static Clr **scheme, color_borderbar;
static Display *dpy;
static Drw *drw;
	...
	if (ev->window == root) {
		...
		if (updategeom() || dirty) {
			...
			for (m = mons; m; m = m->next) {
				...
				XMoveResizeWindow(dpy, m->barwin, m->wx, m->by, m->ww, bh + bar_borderpx);
...
drawbar(Monitor *m)
{
	int x, w, tw = 0, mw, ew = 0, etwl = 0, etwr = 0;
...
//	drw_setscheme(drw, &color_borderbar);
//	drw_rect(drw, 0, bh, m->mw, bar_borderpx, 1, 0);

	/* status bar is drawn on all monitors */
...
setup(void)
{
	...
	drw_clr_create(drw, &color_borderbar, col_borderbar, OPAQUE);
	/* init bars */
	updatebars();
	updatestatus();
